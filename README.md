# benzIA — gateway multiusuario para LM Studio

benzIA pone una capa compatible con la API de OpenAI delante del servidor local de LM Studio. Entrega claves independientes y revocables, contabiliza tokens por identidad, mide latencia y aciertos de caché, y ofrece un panel web sin enviar prompts ni respuestas a servicios externos.

## Puesta en marcha

Requisitos: Node.js 20 o superior y el servidor local de LM Studio activo.
...
También se admite `x-api-key`. Todas las rutas `/v1/*` se reenvían a LM Studio, por lo que funcionan `/v1/models`, completions, embeddings y otros endpoints compatibles.

## Chat web para usuarios

`/chat` ofrece una interfaz de conversación para probar los modelos cargados en LM Studio. Cada usuario debe introducir una clave activa creada en **Claves API**. La pantalla valida esa clave antes de consultar `/v1/models` y cada respuesta se solicita a `/v1/chat/completions` usando el endpoint público configurado en el dashboard.
...
El chat web usa `/v1/responses` con `store: false`: conserva el historial únicamente en el navegador y permite que LM Studio reporte los tokens reutilizados por su prompt cache. Los clientes externos pueden seguir usando todos los endpoints OpenAI-compatible del gateway.
...
- Cuando LM Studio entrega `usage`, benzIA conserva sus contadores exactos.
...
- La caché de prompt/KV de LM Studio se contabiliza aparte cuando el upstream devuelve `cached_tokens` (por ejemplo, en `/v1/responses`). Si el endpoint no lo incluye, el panel lo indica como no reportado.
- Durante una respuesta en streaming, el dashboard muestra el estado de emisión y una velocidad aproximada. Al terminar se conserva `tokens_per_second` si LM Studio lo reporta; en caso contrario se calcula con los tokens de salida y el tiempo de generación observado.
...
| `LM_STUDIO_BASE_URL` | `http://127.0.0.1:1234` | Servidor de LM Studio |
...
El compose expone el panel sólo en localhost y usa `host.docker.internal` para llegar a LM Studio en el host.
